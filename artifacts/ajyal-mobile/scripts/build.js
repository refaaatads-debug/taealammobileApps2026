...
function prepareDirectories(timestamp) {
  ...
  const timestampDirs = [
    path.join(timestampDir, '_expo', 'static', 'js', 'ios'),
    path.join(timestampDir, '_expo', 'static', 'js', 'android'),
  ];

  for (const dir of [...timestampDirs, ...dirs]) {
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  const initialPort = metroPort;
  while (metroPort < initialPort + 20 && await isPortInUse(metroPort)) {
    metroPort += 1;
  }
