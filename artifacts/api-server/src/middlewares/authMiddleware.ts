...
import { getSupabaseUser, readBearerToken, toAuthUser } from "../lib/supabaseAuth";
...
export async function authMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  if (supabaseToken) {
    const supabaseUser = await getSupabaseUser(supabaseToken);
    if (supabaseUser) req.user = toAuthUser(supabaseUser);
    next();
    return;
  }
